...
first_days = data.frame(
  ...
  first_day_c = sprintf('2018-%02d-01', month_int),
...
df = df %>% filter(year(timestamp_c)==2018) %>%
  mutate(run_no = 1:n())
...
# hour
png('hour_2018.png', height=720, width=720, res=PNG_RES)
print(
  ggplot(hour_frame) + 
    ...
    xlab('Hour of Day') + 
    ylab('Number of Runs') + 
    ggtitle('Number of Runs Occuring During Hours of Day in 2018')
)
dev.off()

# hour
png('dow_2018.png', height=840, width=840, res=PNG_RES)
print(
  ggplot(dow_frame) + 
    ...
    xlab('Day of Week') + 
    ylab('Number of Runs') + 
    ggtitle('Number of Runs Occuring During Days of Week in 2018')
)
dev.off()

# hour + dow
png('dow_hour_2018.png', height=840, width=840, res=PNG_RES)
print(
...
png('weekly_distance_2018.png', height=960, width=840, res=PNG_RES)
print(
  ggplot(df %>% inner_join(df_week_summary, by='week_start', suffix=c('','_weekly'))) + 
    ...
    ggtitle('2018 Weekly Total Distance, Colored by Average Pace', subtitle='distances of individual runs indicated by dots')
)
dev.off()

png('weekly_pace_2018.png', height=960, width=840, res=PNG_RES)
print(
  ggplot(df %>% inner_join(df_week_summary, by='week_start', suffix=c('','_weekly'))) + 
    ...
    ggtitle('2018 Weekly Average Pace, Colored by Total Distance', subtitle='paces of individual runs indicated by dots')
)
dev.off()

png('pace_vs_hr_2018.png', height=720, width=720, res=PNG_RES)
print(
  ggplot(df) + 
    ...
    ggtitle('Pace vs. Heart Rate for Runs in 2018', subtitle='approx. 5 bpm per min/mi increase in speed')
)
dev.off()
